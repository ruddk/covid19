...
if uploaded_confirmed and uploaded_deaths and uploaded_recovered:
    ...
    df_confirmed = pd.read_csv(uploaded_confirmed) # 확진자 데이터 프레임
    df_deaths = pd.read_csv(uploaded_deaths) # 사망자 데이터 프레임
    df_recovered = pd.read_csv(uploaded_recovered) # 회복자 데이터 프레임
    ...
    # 탭3 개 구성 (감염 추이, 통계 요약, 비율 분석)
    tab1, tab2, tab3 = st.tabs(['감염추이', '통계요약','비율분석'])

    with tab1:
        st.subheader('누적 추이 그래프')
        selected = st.multiselect('표시할 항목을 선택하세요',['확진자','사망자','회복자'],default=['확진자','회복자']) # 첨 기본값
        if selected: # 선택된 값이 있다면 -> 그래프를 그린다. 
            fig = px.line(df_merged, x='날짜', y=selected, markers=True)
            st.plotly_chart(fig, use_container_width=True)

        st.subheader('일일 증가량 그래프')
        selected_new= st.multiselect('표시할 항목(신규)을 선택하세요',
        ['신규 확진자','신규 사망자','신규 회복자'],default=['신규 확진자'])
        if selected_new:
    ...
    with tab3:
        ...
        pie_df = pd.DataFrame({
            '구분':['회복자','사망자','격리중'],  # 격리중 : confirmed-recovered-deaths
            '인원수':[recovered, deaths, confirmed-recovered-deaths]
            })
        fig_pie = px.pie(pie_df, names = '구분', values='인원수', title='감염자분포')
